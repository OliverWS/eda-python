...
class qPro:
    ...
    def importProject(self,path):
        ...
        for datasetNode in manifest.getElementsByTagName("Dataset"):
            ...
            for eventNode in datasetNode.getElementsByTagName("Event"):
                ...
                try:
                    note = str(eventNode.getElementsByTagName("Notes")[0].firstChild.wholeText)
                except exceptions.Exception as e:
                    log.exception("%s",str(e))
                    note = ""
                isDeviceEvent = True if str(eventNode.getAttribute("device")) == "true" else False
                if not end in map(lambda x: x.start, dataset.events) or end == None:
        ...
        for eventsNode in manifest.getElementsByTagName("Timeline")[0].getElementsByTagName("Events"):
            for eventNode in eventsNode.getElementsByTagName("Event"):
                ...
                try:
                    ...
                    log.exception("Problem parsing note: %s"%(str(e)))
                    note = ""
                isDeviceEvent = False
    ...
    def xmlForDataset(self, dataset):
        defaultDatasetNode = '''<Dataset accel="true" alignmentOffset="0" alignmentStart="-1" color="%s" visible="true"><File>%s</File><Events></Events></Dataset>'''%(self.colors[self.files.index(dataset)], dataset.filename)
